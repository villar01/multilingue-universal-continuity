...
describe("backup audiovisual aprovado de James", () => {
  ...
  it("mantém a faixa masculina de reserva e bloqueia o clipe quando não há par audiovisual exato", () => {
    ...
    expect(sceneSource).toContain("if (!clip || !clip.audioVideoExactPair) return null;");
    expect(sceneSource).toContain('teacherMedia.mode === "pre_generated_video"');
    expect(sceneSource).not.toContain('(teacherMedia.mode === "pre_generated_video" || teacherMedia.mode === "audio_timed_motion_video")');
    expect(sceneSource).toContain("JAMES_TROPICAL_INTRO_FALLBACK_URL,");
  });

  it("mantém o retrato original quando a apresentação não possui vídeo com áudio idêntico", () => {
    const greeting = JAMES_TROPICAL_PILOT_CLIPS.find((clip) => clip.id === "james-tropical-greeting");
  ...
  it("só expõe um vídeo quando a estratégia o classifica como mídia pré-gerada exata", () => {
    expect(sceneSource).toContain('teacherMedia.mode === "pre_generated_video"');
