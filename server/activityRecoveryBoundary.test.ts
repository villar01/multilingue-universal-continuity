...
describe("recuperação local de atividades", () => {
  ...
  it("protege onboarding, IA nativa, monitor de IA, detecção de idioma, controle parental, guia de backup, demonstração, suporte privado, centro de controle, atualizações administrativas, painel, seleção de idiomas, ranking, conquistas, históricos, certificados, chat, perfil docente, progresso, desafios, vídeos, clipes, Reels, aprendizado, revisões, Pareto, diálogo, roleplay, batalha e experiências de realidade aumentada com a fronteira local", () => {
    ...
    expect(app).toContain("const ResilientAiChat");
    expect(app).toContain('<Route path={"/chat"} component={ResilientAiChat} />');
    expect(app).toContain('<Route path={"/ai-chat"} component={ResilientAiChat} />');
    expect(app).toContain("const ResilientMyTeacher");
    expect(app).toContain('<Route path="/my-teacher" component={ResilientMyTeacher} />');
    expect(app).toContain("const ResilientStudentProgress");
    expect(app).toContain('<Route path="/progress" component={ResilientStudentProgress} />');
    expect(app).toContain("const ResilientDailyChallenge");
    expect(app).toContain('<Route path="/daily-challenge" component={ResilientDailyChallenge} />');
    ...
    expect(app).toContain("const ResilientRanking");
    expect(app).toContain('<Route path="/ranking" component={ResilientRanking} />');
    expect(app).toContain("const ResilientAchievements");
    expect(app).toContain('<Route path="/achievements" component={ResilientAchievements} />');
    expect(app).toContain("const ResilientLessonHistory");
    expect(app).toContain('<Route path="/lesson-history" component={ResilientLessonHistory} />');
    expect(app).toContain("const ResilientCertificates");
    expect(app).toContain('<Route path="/certificates" component={ResilientCertificates} />');
    expect(app).toContain("const ResilientPronunciationHistory");
    ...
    expect(app).toContain("const ResilientAIMonitor");
    expect(app).toContain('<Route path="/ai-monitor" component={ResilientAIMonitor} />');
    expect(app).toContain("const ResilientLanguageDetect");
    expect(app).toContain('<Route path="/language-detect" component={ResilientLanguageDetect} />');
    expect(app).toContain("const ResilientParentalControl");
    expect(app).toContain('<Route path="/parental-control" component={ResilientParentalControl} />');
    expect(app).toContain("const ResilientBackupGuide");
    ...
    expect(app).toContain("const ResilientAdminControlCenter");
    expect(app).toContain('<Route path="/admin/control-center" component={ResilientAdminControlCenter} />');
    expect(app).toContain("const ResilientAdminUpdates");
    expect(app).toContain('<Route path="/admin/updates" component={ResilientAdminUpdates} />');
    ...
    expect(app).toContain('<Route path="/clips" component={ResilientClips} />');
