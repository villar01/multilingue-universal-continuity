...
const [languages] = await conn.query('SELECT id, code, name FROM languages ORDER BY code LIMIT 10');
...
// Para cada idioma, criar 3 cursos
for (const lang of languages) {
  console.log(`\n📖 ${lang.name} (${lang.code})`);
  
  for (const level of levels) {
    try {
      // Criar curso (SEM isActive)
      ...
      courseCount++;
      console.log(`  ✅ Curso ${level}: ID ${courseId}`);
      
      // Criar 10 lições para este curso
