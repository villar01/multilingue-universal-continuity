...
- [ ] Diagnosticar a falha audível persistente de James com telemetria local do player: após gesto explícito, registrar somente no console de desenvolvimento a existência da faixa, `muted`, volume, duração, evento `play`, rejeição de `play()` e `error`, sem mostrar termos técnicos ao aluno
