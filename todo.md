...
- [x] Bloquear a publicação de alterações da Cena Imersiva sem regressão de renderização, TypeScript, suíte aprovada e verificação funcional autenticada registrada — o build exige tipagem, renderização headless autenticada da rota, recuperação específica e suíte inteira antes de gerar a aplicação; TypeScript, 431 arquivos / 1067 testes e build completo aprovados
- [x] Adicionar ao portão de publicação uma verificação autenticada registrada da rota /immersive-scene e falhar o build se ela não for aprovada — navegador headless abre a página pública, navega internamente para a Cena com sessão e aceite simulados, e confirma Praia Tropical, retrato de James e controle de apresentação antes do build
- [x] Exigir no portão uma regressão específica de renderização autenticada da Cena Imersiva, além da suíte genérica e dos contratos estáticos — o portão executa a checagem headless autenticada e as regressões críticas de fluxo/recuperação antes da suíte completa
