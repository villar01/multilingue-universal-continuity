...
- [ ] Integrar ciclo Pareto em todo o app: vocabulário útil, recuperação ativa, escrita, memorização e criação de novas frases por nível — concluído na lição guiada, hotspots, painel de vocabulário das cenas, clipes familiares A1 e treino diário; os demais fluxos de prática ainda precisam de integração
...
- [x] Aplicar a mesma guarda etária, consentimento e alerta parental aos fluxos restantes de conversa livre, roleplay e geração de frases — `ai.freeChat`, `vrConversation` (abertura, resposta e livre), aventura/roleplay, chat da lição, chat da cena e editor de frase agora exigem perfil autenticado, avaliam entrada/saída pela guarda central e retornam somente fallback pedagógico seguro quando bloqueados; alertas vinculados preservam apenas categoria, nunca texto; TypeScript e 131 testes aprovados
