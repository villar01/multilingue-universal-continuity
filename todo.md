...
- [ ] Ocultar objetos interativos da Cena Imersiva quando o material protegido estiver indisponível ou inconsistente, evitando que a interface contradiga a ativação de acesso — hotspots, cartões, prática, quiz e fala docente já exigem diálogo canônico autorizado; a revisão de todos os controles de Pareto e a verificação funcional específica permanecem pendentes
- [ ] Bloquear explicitamente todos os controles e entradas de Pareto da Cena Imersiva enquanto `sceneMaterialNeedsAccess` for verdadeiro e cobrir a guarda com regressão dedicada
- [x] Bloquear o botão, o painel e a prática Pareto da Cena Imersiva até o material canônico estar autorizado — estado aberto é encerrado ao perder autorização; regressões de prévia e fluxo crítico, TypeScript e 358 arquivos/901 testes aprovados
- [ ] Adicionar verificação funcional dedicada da Cena Imersiva comprovando que, quando o acesso protegido falha ou fica inconsistente, não aparecem hotspots, cartões, quiz, Pareto nem fala docente
- [ ] Corrigir a mensagem da Cena Imersiva que pede ativação de acesso mesmo quando o material protegido já foi autorizado
- [x] Corrigir a orientação da Cena Imersiva após autorização do material — a instrução passa a convidar à exploração e prática quando o acesso está ativo, mantendo ativação apenas no estado pendente; TypeScript e 358 arquivos/901 testes aprovados
...
- [x] Adicionar teste de integração real da rota `/lesson/:id` em erro, comprovando fallback local, saídas seguras e ausência de acionamento da fronteira global — o teste monta `Router` na URL `/lesson/42` sob a fronteira global real, força a falha em `Lesson` e confirma que a recuperação local assume a tela sem “Algo deu errado”
- [x] Renderizar o roteador real em `/lesson/:id`, forçar um erro no componente `Lesson` e confirmar que a `LessonRecoveryBoundary` assume o fallback sem acionar a fronteira global — recuperação temporizada e fallback final comprovados; TypeScript e 896 regressões aprovados
...
- [x] Corrigir as contagens contraditórias de idiomas da página de pré-lançamento usando o catálogo canônico compartilhado — página deriva 143 no catálogo, 58 disponíveis agora e 85 em preparação; TypeScript, regressão dedicada e verificação visual aprovados
- [x] Corrigir a contagem contraditória de idiomas na página de preços, substituindo o rótulo público de 54 idiomas pelas contagens canônicas do catálogo — cabeçalho e planos informam 143 no catálogo, 58 ativos e 85 em preparação; TypeScript, regressão de claims, verificação visual e 358 arquivos/900 testes aprovados
...
- [x] Verificar de forma não bloqueante cada snapshot automático concluído — o callback cron exige status concluído, metadados íntegros, tamanho positivo e URL de objeto disponível; falhas alertam o proprietário sem interromper alunos nem reativar qualquer rotina pausada; TypeScript e 358 arquivos/900 testes aprovados
- [x] Comprovar o acúmulo idempotente dos snapshots automáticos por janela — a mesma janela reutiliza o snapshot já concluído, enquanto a janela seguinte grava uma cópia distinta; TypeScript e 358 arquivos/901 testes aprovados
