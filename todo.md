...
- [ ] Eliminar a invalidação de atualização a quente de ImmersiveScene movendo o catálogo estático das 29 cenas e referências visuais para módulo sem componente, sem alterar professores, mídia ou conteúdo pedagógico
- [x] Evitar a mensagem de incompatibilidade da atualização a quente de ImmersiveScene com recarregamento seguro da página durante mudanças de desenvolvimento, preservando o catálogo estático atual — diretiva `@refresh reset` aplicada e protegida por regressão; mudança seguinte gerou atualização da página sem invalidação incompatível; TypeScript e suíte completa com 772 testes aprovados
...
- [ ] Criar regressão de backup audiovisual: a rota ativa de James deve apontar ao clipe lateral previamente validado e à faixa masculina de reserva; um ativo experimental não pode se tornar padrão apenas por existir no armazenamento
