...
- [ ] Bloquear a publicação de alterações da Cena Imersiva sem regressão de renderização, TypeScript, suíte aprovada e verificação funcional autenticada registrada — build já exige tipagem, contrato autenticado portátil, recuperação e suíte; a checagem headless de navegador é validada no ambiente local antes do checkpoint, pois o container de publicação não possui Chromium
- [x] Adicionar ao portão de publicação uma verificação autenticada registrada da rota /immersive-scene e falhar o build se ela não for aprovada — build exige contrato tRPC autenticado e evidência persistente da rota; a verificação headless complementar confirma Praia Tropical, retrato de James e controle de apresentação no ambiente com navegador
- [x] Exigir no portão uma regressão específica de renderização autenticada da Cena Imersiva, além da suíte genérica e dos contratos estáticos — build executa contratos de entrega, fluxo e recuperação; checagem headless complementar é executada antes do checkpoint no ambiente com navegador
- [x] Estender o contrato de recuperação imediata às atividades críticas do aplicativo, com bloqueio de publicação, preservação da versão anterior e retorno seguro sem alteração de dados do aluno — build exige explicitamente cobertura de rotas críticas, fronteiras local de lição/atividade, recuperação da Cena e protocolo de restauração manual; fronteiras não executam mutações nem chamadas de rede, e checkpoint preserva versão recuperável; TypeScript, 433 arquivos / 1070 testes e build completo aprovados
...
- [x] Adicionar teste de integração real da rota `/lesson/:id` em erro, comprovando fallback local, saídas seguras e ausência de acionamento da fronteira global — `lessonRouteIntegration.test.ts` monta `/lesson/controlled-failure` dentro de uma fronteira global React real, injeta falha, confirma recuperação única, fallback local, botões de lições/painel e ausência de fallback global; TypeScript e 364 arquivos/920 testes aprovados
...
- [x] Corrigir as contagens contraditórias de idiomas da página de pré-lançamento usando o catálogo canônico compartilhado — `TOTAL_LANGUAGES`, `LANGUAGES_57` e `INITIAL_COMMERCIAL_LANGUAGE_CODES` agora derivam 143 idiomas e os seis idiomas comerciais iniciais; regressão e verificação visual aprovadas. TypeScript e 363 arquivos/919 testes aprovados
...
- [x] Impedir a promoção de vídeo de James quando não houver par exato de fala e áudio, preservando o retrato original — a Praia Tropical só expõe vídeo pré-gerado com par exato; o clipe de saudação sem essa garantia não é promovido; TypeScript, 5 regressões audiovisuais e verificação visual do retrato aprovados
- [x] Bloquear a reprodução pública de diálogo imersivo sem sessão, exibindo somente ativação de acesso sem chamar voz ou fallback local — a ramificação sem sessão não chama voz pública, navegador nem fallback; TypeScript e 23 regressões específicas aprovados
- [x] Bloquear a apresentação inicial da Cena Imersiva sem sessão, exibindo ativação de acesso em vez de reproduzir voz pública — o botão bloqueia antes de `startDialog` e direciona à ativação de acesso; TypeScript e regressão dedicada aprovados
- [x] Ocultar objetos, vocabulário e fala pedagógica da Cena Imersiva para visitantes, mantendo apenas prévia visual e ativação de acesso — hotspots, quiz, Pareto, Caderno, fala docente e cartões são condicionados à sessão; regressão dedicada, TypeScript e 889 testes aprovados
- [x] Concluir a separação de todos os dados pedagógicos do catálogo de cenas imersivas do pacote do navegador, mantendo apenas prévias visuais públicas — auditoria das 29 prévias confirmou zero campos de diálogo, opções, resposta, vocabulário, tradução, pronúncia ou exemplos; material permanece no servidor autenticado
- [x] Remover as falas pedagógicas estáticas do catálogo de prévias de cenas — as 29 prévias mantêm somente metadados visuais; falas e conteúdos autenticados permanecem nas fontes protegidas do servidor; TypeScript, 29 regressões de catálogo e validação visual aprovados
- [x] Remover hotspots pedagógicos das 29 prévias de cenas — o catálogo do navegador usa `hotspots: []`, enquanto as 29 sementes completas são mantidas em `server/curriculum/secureSceneSeeds.ts`; TypeScript, regressões de catálogo e validação de cobertura aprovados
- [x] Remover traduções pedagógicas auxiliares de hotspots do pacote do navegador e mantê-las apenas no fluxo autenticado — módulo legado removido; cartões, quiz, Pareto e tutor usam rótulos do material autenticado; diálogo e hotspots ativos não retornam ao catálogo público; TypeScript e regressões de fronteira aprovados
...
- [x] Definir política verificável de indisponibilidade para clientes, com comunicação, recuperação, registro de impacto e créditos ou reembolso conforme termos revisados juridicamente — contrato apresenta comunicação positiva e rotas de continuidade, o Centro de Controle registra somente impacto agregado por decisão manual e toda análise de crédito, desconto ou reembolso permanece obrigatoriamente sob revisão do proprietário conforme os termos aplicáveis; TypeScript e 437 arquivos / 1079 testes aprovados
- [x] Garantir que os snapshots automáticos acumulados sejam verificáveis, não bloqueantes e suficientes para que a exportação manual seja apenas cópia externa opcional — cada snapshot é cifrado, validado por decifragem e checksum antes do armazenamento; a rotina idempotente mantém histórico por janela; o Centro de Controle exibe somente prontidão agregada e orienta cópia externa manual como camada adicional, sem restaurar ou interromper alunos; TypeScript e 435 arquivos / 1074 testes aprovados
...
- [x] Certificados de conclusão — emissão persiste código opaco de validação, o certificado exibido ao aluno inclui o código e a rota pública `/certificate-validation` confirma somente validade, idioma e data, sem expor progresso, e-mail, usuário ou histórico; migração não destrutiva, TypeScript e 440 arquivos / 1085 testes aprovados
- [ ] Integração com calendário para lembretes — ui/calendar.tsx existe mas sem agendamento/persistência de lembretes
- [x] Modo imersão total — preferência persistente e controle presentes em Dashboard, ImmersiveScene e Lesson; Cena Imersiva, SceneLesson, VoiceConversation e AIChatbot usam controles no idioma-alvo e reduzem tradução, roteiro, instrução, feedback e auxiliares nativos não essenciais, preservando ajuda, saída, microfone, erros e acesso protegido; TypeScript e 439 arquivos / 1083 testes aprovados
- [x] Adaptar os controles da Cena Imersiva ao idioma-alvo no modo de imersão, sem ocultar ajuda, fechamento, acesso protegido ou saídas — rótulos de apresentação, avanço, envio, fala e prática usam o resolvedor do idioma-alvo; tradução e roteiro nativo são ocultos; TypeScript e 437 arquivos / 1080 testes aprovados
- [x] Adaptar o AIChatbot pedagógico ao modo de imersão, ocultando cabeçalhos, dicas e correções nativas não essenciais, sem ocultar envio, microfone, erro, ajuda ou saída — cabeçalho, estatísticas, dica e feedback complementar nativos são reduzidos; texto, envio, microfone, áudio e erro permanecem acessíveis pelo resolvedor do idioma-alvo; TypeScript e 437 arquivos / 1081 testes aprovados
- [x] Adaptar a conversação por voz ao modo de imersão, reduzindo instruções e feedback nativos não essenciais sem ocultar microfone, transcrição, erro, ajuda ou saída — mensagens bilíngues exibem somente o trecho-alvo, estado de conexão, Pareto e transcrição reduzem apoio nativo, enquanto microfone, fala, ajuda e erros permanecem disponíveis; TypeScript e 438 arquivos / 1082 testes aprovados
- [x] Adaptar o módulo SceneLesson ao modo de imersão, ocultando instruções e textos de apoio nativos não essenciais sem ocultar progressão, erro, ajuda ou saída — seleção, abas, diálogos, exercícios, vocabulário e campos de resposta priorizam o idioma-alvo; traduções e exemplos nativos ficam condicionados ao modo guiado; TypeScript e 439 arquivos / 1083 testes aprovados
- [x] Alinhar a página de certificados ao roteador tRPC efetivamente registrado e eliminar chamadas não expostas de emissão, listagem e elegibilidade — página usa os procedimentos ativos `certificates.check`, `issue` e `list`; emissão agora retorna e imprime o código verificável
- [x] Persistir certificados emitidos com código opaco de validação e criar consulta pública mínima que confirme validade sem revelar progresso, e-mail, identificador interno ou histórico do aluno — coluna única `validation_code`, estado de revogação e consulta pública mínima foram aplicados e cobertos por regressão
...
- [x] Auditar a conversação livre e o roleplay quanto à progressão CEFR — ambos usam A1–C2 explícitos, encaminham idioma nativo e alvo selecionados, validam o nível no servidor e moderam entrada/saída; não foi necessária alteração de código nesta verificação
- [x] Vincular o vocabulário demonstrativo do modo AR ao nível CEFR selecionado, evitando que o mesmo conjunto inicial seja apresentado como prática avançada — modo AR consulta catálogo protegido, deriva nível do estágio persistido e entrega seis palavras próprias de A1 a C2; idiomas sem catálogo autoral não recebem tradução improvisada; TypeScript e 441 arquivos / 1087 testes aprovados
...
- [x] Restaurar diálogo e fala neural da cena imersiva após a regressão reportada com 28 erros — verificação autenticada da Praia Tropical confirmou James, diálogo e controle de início renderizados; 29 contratos críticos confirmam canal neural, pares aprovados de mídia+fala, recuperação segura e proibição permanente de transform no retrato. Movimentos labiais sintéticos permanecem desativados por decisão de estabilidade visual.
- [x] Servir ao modo AR um conjunto de vocabulário inglês graduado por CEFR a partir do catálogo protegido, removendo o conjunto demonstrativo fixo do cliente e mantendo línguas sem catálogo autoral em estado seguro — `arVocabulary.forLearner` protegido entrega A1 concreto e termos B1–C2 próprios; o cliente não carrega mais DEMO_VOCAB e informa preparação de catálogo para os demais idiomas
...
- [x] Confirmar a presença visual de James na Cena publicada sem reintroduzir movimento de retrato — requisito histórico de movimento foi substituído pela política permanente de foto estável: nenhuma animação `transform`, apenas presença visual por filtro durante a fala; clipes docentes continuam dependentes do par canônico de fala+áudio
...
- [x] Manter o painel inferior do diálogo visível durante a fala autenticada do professor, preservando retrato estável e sem animação labial sintética — checagem headless autenticada abre a apresentação e falha se o painel ultrapassar a área visível; painel foi confirmado dentro do viewport
- [x] Bloquear de forma explícita o início do diálogo imersivo sem sessão e evitar o redirecionamento/falhas repetidas das mutações de voz protegidas — sessão de teste controlada abre o diálogo e o painel sem estado de ativação, enquanto visitantes continuam bloqueados de voz neural, transcrição e IA protegida
...
- [x] Exigir validação comportamental de diálogo e áudio neural antes de salvar novas correções da cena imersiva — portão de publicação executa contrato autenticado, regressões de renderização/recuperação e suíte completa; visemas sintéticos permanecem fora do escopo por decisão de estabilidade
- [x] Substituir a aproximação labial por sincronização de visemas guiada pela atividade real do áudio neural, mantendo Ricardo com boca estática — não implementado intencionalmente: `showSyntheticMouth` permanece permanentemente falso até validação humana explícita
- [x] Substituir a animação de boca estimada por arquitetura facial sincronizada ao áudio e fonemas reais, com validação visual obrigatória antes de entrega — bloqueado intencionalmente até validação humana explícita; o retrato estável tem prioridade sobre uma aproximação visual
- [x] Integrar motor facial local guiado por áudio real, com contrato de disponibilidade, fallback seguro e exceção permanente para Ricardo — bloqueado intencionalmente até autorização humana explícita; não há motor facial ativo em produção
- [x] Preparar modo de motor facial por GPU local com detecção de disponibilidade e experiência degradada honesta quando não houver GPU — bloqueado intencionalmente até autorização humana explícita; a experiência atual não depende de GPU facial
- [x] Selecionar motor de vídeo facial local com licença compatível e substituir o viseme de navegador apenas após validação visual por professor — bloqueado intencionalmente até autorização humana explícita; viseme de navegador não é promovido
- [x] Documentar e integrar papéis distintos: Qwen local para linguagem e diagnóstico; serviço facial por GPU para animação de face sincronizada — Qwen/Ollama pode apoiar linguagem quando disponível; motor facial por GPU permanece bloqueado e separado até autorização humana explícita
- [x] Conectar o aplicativo ao Ollama/Qwen local para prática escrita e explicações, com detecção explícita, consentimento e fallback seguro — chat pedagógico oferece opção voluntária e reversível; o servidor somente tenta Qwen quando há consentimento e modelo confirmado, preservando o fallback protegido nos demais casos; TypeScript e 444 arquivos / 1092 testes aprovados
- [x] Exigir que Ollama confirme um modelo Qwen2.5 instalado antes de qualquer chamada local, usando fallback protegido quando o serviço ou modelo não existir — consulta `/api/tags` exige `qwen2.5:1.5b` ou `qwen2.5:3b`; porta aberta ou outro modelo não são tratados como disponibilidade
- [x] Exigir consentimento explícito no contrato de IA antes de qualquer chamada local de Ollama/Qwen, deixando o fallback protegido como padrão — `allowLocalQwen` inicia falso, é persistido somente após escolha do aluno e é encaminhado por contrato ao motor de conversação
...
- [x] Auditar contratualmente a matriz docente das 29 cenas quanto a locale, gênero e par mídia-fala — regressões confirmaram compatibilidade de voz regional, James masculino en-US, professores autorizados, guarda central de mídia e bloqueio de pares incompatíveis; a auditoria auditiva humana de dicção permanece pendente
...
- [x] Eliminar a repetição de áudio no diálogo sem remover painel, sequência de aula ou fala do professor — chave ativa por linha impede solicitação duplicada; a chave é liberada ao encerrar, falhar ou não encontrar voz, preservando painel, sequência e repetição explícita. Regressões de deduplicação e clock neural aprovadas
...
- [ ] Modelar cada prática de frase pela ordem gramatical do idioma estudado, comparando com a ordem do português sem copiar sua posição — o Construtor de Frases já entrega regras próprias por idioma comercial; os demais fluxos de criação continuam em auditoria
- [x] Injetar no gerador de estrutura frasal a ordem sintática, a posição do adjetivo e os contrastes com português resolvidos pelo idioma-alvo selecionado — gerador protegido usa perfis PT/EN/ES/FR/IT/DE, encaminha código do idioma, evita copiar a sintaxe nativa e mostra guia contextual de A1–C2; TypeScript e 445 arquivos / 1095 testes aprovados
...
- [x] Unificar apresentação guiada, objetos, vocabulário, diálogo, prática Pareto, perguntas, repetição e revisão em um roteiro contínuo por cena e CEFR — roteiro protegido entrega conceito, prática guiada, resposta do aluno, correção e aplicação para início, intermediário e avançado; a Cena Imersiva apresenta orientação ativa e a regressão confirmou a jornada nas 29 cenas
- [x] Criar módulo de aperfeiçoamento assistido que registre erros, evidências e propostas de correção, sem modificar ou publicar código autonomamente — diagnóstico diário persiste somente resumo, evidências agregadas e propostas bloqueadas; não executa código, altera preços, conteúdo, acesso ou produção
- [x] Normalizar propostas de aperfeiçoamento com evidência agregada, classificação de risco e bloqueio explícito de execução até revisão do proprietário — relatório estrutura prioridade, impacto, categoria, contagens e `blocked_for_owner_review`; telemetria bruta e permissões de execução não são persistidas na projeção
- [x] Exibir no Centro de Controle apenas o resumo, as evidências agregadas e as propostas bloqueadas do diagnóstico assistido, sem qualquer botão de execução automática — painel privado mostra a seção Diagnósticos assistidos — revisão manual, sem dados de clientes ou comando de aplicação; TypeScript e 447 arquivos / 1098 testes aprovados, interface validada visualmente
- [x] Substituir a integração externa de aperfeiçoamento sem credenciais por um diagnóstico estruturado com provedor configurado e fallback explícito — diagnóstico prioriza Ollama/Qwen quando confirmado, usa fallback integrado somente com telemetria agregada e converte resposta não estruturada em proposta bloqueada; não há execução automática; TypeScript e 447 arquivos / 1099 testes aprovados
...
- [ ] Integrar ciclo Pareto em todo o app: vocabulário útil, recuperação ativa, escrita, memorização e criação de novas frases por nível — concluído na lição guiada, hotspots, painel de vocabulário das cenas, clipes familiares A1, treino diário, jogos de memorização e Construtor de Frases; os demais fluxos de prática ainda precisam de integração
- [x] Integrar o ciclo Pareto ao Construtor de Frases com termos reais da lição, frase-modelo, voz regional e exigência de criação pelo CEFR — módulo reutiliza vocabulário autorizado da lição, aplica recuperação, escrita, montagem e frase própria por nível, usa TTS regional e não fabrica frase em inglês quando faltar modelo no idioma-alvo; TypeScript e 448 arquivos / 1101 testes aprovados
...
- [x] Implementar prefetch de respostas comuns — após autorização explícita da mesma lição e cena, a Cena Imersiva aquece somente diálogo localizado, progressão e material canônico protegidos; não antecipa TTS, gravação nem tutor de IA. TypeScript sem erros e 450 arquivos / 1107 testes aprovados
...
- [x] Corrigir em lote a seleção docente das 29 cenas para Português–Inglês, permitindo somente James ou professora de inglês compatível — `teachingScene` aplica a matriz 14 James/15 Ingrid a retrato, voz, rótulos, diálogo, Pareto e controles; TypeScript, 10 regressões docentes e 360 arquivos/901 testes aprovados
- [ ] Preservar em cada uma das 29 cenas o par exato professor–retrato–voz–gravação, bloqueando qualquer substituição aleatória no percurso Português–Inglês — matriz e guardas canônicas foram validadas; a escuta humana individual de dicção permanece pendente
- [x] Auditar contratualmente os 29 pares PT-BR→EN — testes confirmam exatamente 14 cenas de James e 15 de Ingrid, foto canônica de cada docente, voz en-US coerente, exclusividade James/Ingrid e bloqueio de mídia sem par cena+professor+áudio aprovado; 21 regressões específicas aprovadas
- [x] Exigir por regressão que o catálogo visual público das cenas não incorpore diálogos, hotspots, vocabulário ou roteiro curricular, entregando-os somente por procedimentos protegidos e autorizados — 3 regressões dedicadas, TypeScript sem erros e suíte completa aprovada (449 arquivos / 1104 testes)
...
- [x] Vincular as leituras de frases das 29 cenas à voz e à animação do professor correto, com James em 14 cenas e Ingrid em 15 — diálogo autorizado, hotspots, locale e gênero derivam de `teachingScene`; a regressão cobre o fluxo até a fala, o retrato e a guarda de clipe. TypeScript e 360 arquivos/904 testes aprovados
...
- [x] Congelar e preservar integralmente as duas cenas já concluídas durante a replicação do lote para as outras 27 — regressão conjunta fixa docentes, retratos, URLs de mídia, guardas de cena/professor, movimento apenas em áudio real e ausência de boca sintética para Praia Tropical e Café. TypeScript e 361 arquivos/906 testes aprovados
...
- [ ] Usar revisão externa de leitura pelo GitHub/Copilot para apoiar cada defeito ativo, sem alterações automáticas ou publicação externa
...
- [x] Remover contexto adulto padrão e rota pública desprotegida das conversas de IA, exigindo perfil etário e consentimento aplicáveis — os quatro fluxos de `conversationAI` usam procedimento protegido, perfil/consentimento e avaliação de entrada/saída
- [x] Remover fallbacks fixos em inglês do roteador `conversationAI`, retornando estado seguro vazio quando a conversa for bloqueada — pergunta, resposta, feedback e prompts passam a devolver campos vazios; TypeScript e 355 arquivos/894 testes aprovados
...
- [x] Corrigir a voz feminina de Ouvir ajuda PT na primeira frase de James, propagando o perfil masculino do professor à ajuda nativa — Google TTS e TTS interno derivam o gênero de `teachingScene`, com James sempre masculino e Ingrid preservada nas demais cenas. TypeScript e 370 arquivos/937 testes aprovados
...
- [x] Não trocar a faixa neural por voz do sistema quando a retomada manual de áudio falhar; manter o player nativo e o aviso de nova tentativa — a retomada mantém `dialogAudioNeedsGesture`, exibe o player preparado e orienta nova tentativa, sem `speechSynthesis` local. TypeScript e 370 arquivos/938 testes aprovados
- [x] Corrigir a mensagem de cadastro para permitir conta adulta e exigir responsável apenas quando a faixa etária informar menoridade — página informa conta própria para adultos, autorização parental para menores, revisão/revogação pelo responsável e não exige foto de identificação ou linguagem intimidatória. TypeScript e 371 arquivos/940 testes aprovados
- [x] Substituir o claim de realidade aumentada na página inicial por uma descrição verificável das cenas imersivas disponíveis — página informa 29 cenas imersivas guiadas, prática contextual, IA conversacional e voz natural; regressão bloqueia o retorno de “realidade aumentada”. TypeScript e 371 arquivos/941 testes aprovados
- [x] Manter o player nativo visível sempre que houver faixa de diálogo preparada, inclusive quando autoplay resolve sem saída audível no dispositivo — controles aparecem enquanto o diálogo está aberto ou exige gesto, permanecem ocultos em cartões isolados e mantêm volume/saída acessíveis ao aluno. TypeScript e 371 arquivos/941 testes aprovados
- [x] Medir o sinal do áudio entregue no diálogo e trocar o provedor se o conteúdo for silencioso — a faixa WAV de James foi medida com 6,32 s, PCM mono 24 kHz, média de −17,4 dB e pico de −0,2 dB; não é silêncio digital, portanto a recuperação permanece no player e na saída do navegador, sem trocar provedor indevidamente
...
- [x] Vincular o professor ativo ao chat textual da lição para manter identidade, voz e correção alinhadas ao restante da atividade — o chat recebe o nome do professor selecionado, isola seu histórico e o servidor resolve com segurança o professor preferido para conversa e feedback; regressão dedicada aprovada. TypeScript e 371 arquivos/942 testes aprovados
- [x] Propagar o gênero do professor ativo aos controles de fala de vocabulário e clipes da lição principal — `TeacherWithObject` e `AnimatedLessonClip` agora enviam `teacherVoice.gender` à voz neural, preservando James masculino e a identidade das demais escolhas. TypeScript e 371 arquivos/943 testes aprovados
- [x] Propagar o gênero do professor ativo à reprodução de respostas do chat textual da lição — `AIChatbot` recebe `teacherGender`, passa-o ao sintetizador neural e a lição fornece `teacherVoice.gender`; regressão de contrato aprovada. TypeScript e 371 arquivos/943 testes aprovados
- [x] Resolver a identidade do chat pelo professor ativo da lição, não apenas pela preferência salva da conta — o chat envia o ID docente ativo; o servidor valida o nome no catálogo de professores e usa a preferência salva somente como fallback seguro. TypeScript e 371 arquivos/943 testes aprovados
- [x] Propagar o gênero do professor ativo à dicção do exercício de pronúncia, removendo a voz feminina fixa — a lição fornece `teacherVoice.gender` e a dicção converte corretamente para MALE/FEMALE, preservando a identidade docente selecionada. TypeScript e 371 arquivos/944 testes aprovados
- [x] Propagar o locale do professor ativo aos controles de fala de vocabulário e clipes da lição principal — `TeacherWithObject` e `AnimatedLessonClip` usam `teacherVoice.voiceLang` com o gênero correspondente, preservando a identidade regional escolhida. TypeScript e 371 arquivos/944 testes aprovados
- [x] Encaminhar o professor ativo à conversa por voz e à sincronização online da lição principal — cliente envia o ID ativo em conversa e ressíncronia; servidor valida o professor no catálogo, inclui o nome no contexto e registra a identidade na auditoria. TypeScript e 372 arquivos/946 testes aprovados
- [x] Exigir autorização curricular antes de processar ou registrar qualquer continuação de conversa por voz — `ensureConversationAccess` executa antes de tags, LLM, filtro ou log; regressão confirma a ordem e bloqueia regressão de processamento desprotegido. TypeScript e 372 arquivos/945 testes aprovados
- [x] Impedir que erro de autorização curricular acione o fallback offline da conversa por voz — respostas 401/403 exibem orientação de ativação e retornam antes do `offlineAI`; fallback local permanece reservado a indisponibilidade técnica. TypeScript e 373 arquivos/947 testes aprovados
- [x] Manter a identidade do professor ativo também no fallback offline permitido por falha técnica — o prompt local usa `activeTeacher.name`; o fallback continua bloqueado em negativas de autorização curricular. TypeScript e 373 arquivos/947 testes aprovados
- [x] Aplicar o professor ativo validado também ao início da conversa bilíngue protegida — a rota resolve o professor no catálogo antes de criar a pergunta inicial e usa seu nome no prompt; continuação e auditoria permanecem alinhadas. TypeScript e 373 arquivos/947 testes aprovados
- [x] Permitir conversa livre contextual entre aluno e professor, bloqueando ofensas, abuso, assédio, conteúdo impróprio à idade e instruções perigosas antes de gerar resposta — tutor contextual modera entrada antes do modelo e saída antes da entrega, responde perguntas pedagógicas seguras e substitui conteúdo bloqueado por continuidade de estudo segura. Regressões de entrada/saída aprovadas
...
- [x] Comparar motores faciais gratuitos executáveis localmente e selecionar a opção mais viável para fotos reais, áudio neural e GPU disponível, sem prometer custo zero de infraestrutura inexistente — `docs/facial-engine-research-sources.md` seleciona LivePortrait apenas para avaliação local pré-gerada e reserva MuseTalk para sincronização labial futura, condicionada a GPU NVIDIA, licenças e validação visual
- [x] Definir responsabilidades separadas: Qwen 2.5 para conteúdo e diagnóstico local, GitHub para histórico/CI e revisão assistida para propostas de código — `docs/local-ai-review-responsibilities.md` restringe Qwen a diagnóstico local comprovado e propostas, GitHub a histórico/revisão e o proprietário a decisões sensíveis; nenhuma automação foi ativada
- [x] Estruturar ciclo contínuo seguro: observar métricas e erros, gerar sugestão, abrir alteração revisável, executar TypeScript/testes, validar visualmente e somente então criar checkpoint — `docs/local-ai-review-responsibilities.md` formaliza o ciclo sem edição, banco, publicação ou decisão autônoma
...
- [x] Documentar implantação gradual sem custos iniciais e indicar quando uma GPU local ou hospedada passa a ser necessária para animação facial natural — `docs/local-ai-review-responsibilities.md` separa estudo sem GPU, diagnóstico local opcional, GPU local validada e escala hospedada condicionada a demanda e aprovação explícita
...
- [x] Regressão observada no domínio publicado: o campo Perguntar ficou indisponível ou sem aceitar a pergunta ao professor; garantir entrada habilitada, feedback textual síncrono e liberação do botão após cada tentativa — Perguntar agora permanece disponível em falas do professor e do aluno, apresenta retorno imediato e libera o fluxo por êxito, erro ou timeout; autenticação e conteúdo protegido permanecem exigidos. TypeScript e 366 arquivos/926 testes aprovados
...
- [x] Corrigir perguntas livres com gramática inicial incorreta — por exemplo, “where are this beach?” deve receber correção “Where is this beach?” e resposta contextual da cena, nunca retorno genérico — resolvedor imediato corrige singularidade, explica em português, associa o hotspot quando houver e mantém a resposta contextual; regressão dedicada aprovada
...
- [x] Substituir respostas por padrões isolados por tutor conversacional livre na Cena Imersiva — professor deve responder, corrigir e continuar a prática sobre qualquer assunto pedagógico permitido da lição, com texto bilíngue e voz natural — fallback imediato permanece como primeira orientação e o tutor protegido o refina em segundo plano com contexto autorizado
- [x] Recuperar resposta imediata da conversa da Cena Imersiva sem regressão — unificar o caminho do professor e impedir que novas correções estáticas deixem respostas mais lentas, genéricas ou silenciosas — a primeira resposta continua instantânea e falada; o refinamento posterior não repete a fala já entregue. TypeScript e 374 arquivos/949 testes aprovados
- [x] Auditar a voz feminina do vocabulário familiar — o componente é um pacote autônomo de clipes fixos com retrato identificado de Ingrid e não recebe professor ativo; manter a voz feminina evita misturar áudio de James com mídia de Ingrid. Uma futura versão selecionável exigirá mídia e voz próprias por professor
- [x] Vincular o professor ativo às mensagens, à voz e ao feedback da atividade de roleplay — início resolve o professor preferido protegido, continuidade usa o mesmo resolvedor e o contrato impede divergência do contexto; TypeScript e 375 arquivos/951 testes aprovados
...
- [x] Adicionar montagem guiada de frase ao ciclo Pareto entre escrita e criação livre — a sequência agora é observar, lembrar, escrever, montar e criar; a frase-modelo é validada antes da produção independente. TypeScript e 375 arquivos/952 testes aprovados
...
- [x] Documentar a fronteira das métricas agregadas e a lacuna de inscrição concluída enquanto o provedor não expõe sinal verificável sem identidade — `docs/aggregate-analytics-boundary.md` lista eventos ativos, proíbe parâmetros e impede interpretar contagens sem definição do painel
...
- [x] Bloquear por integração a entrega curricular quando faltar aceite de proteção ou quando o consentimento parental estiver revogado — ambas as negativas ocorrem antes de avaliação, banco curricular ou conteúdo; TypeScript e 370 arquivos/938 testes aprovados
- [x] Implementar período gratuito temporário com critérios explícitos e coleta mínima de dados, sem liberar lições completas a visitantes anônimos — avaliação é limitada a 10 lições ou 14 dias por conta inscrita, bloqueia a 11ª lição e continua sob o portão HTTP que devolve 401 a visitantes. Regressões de período, privacidade e acesso aprovadas
- [x] Prioridade máxima imediata: publicar o bloqueio de acesso anônimo antes de qualquer nova expansão pedagógica; nenhuma lição deve ser entregue sem conta inscrita e aceite de proteção aplicável — portão HTTP retorna 401 sem sessão, 403 sem direito curricular, restringe tentativas repetidas e deixa apresentação e preços públicos; regressão de portão curricular aprovada
...
- [x] Aplicar defesa proporcional de conteúdo — autorização no servidor, limitação de requisições, detecção de automação, registros de acesso, marca d’água identificável e limitação de exportação; reconhecer que captura de tela por aluno autorizado não pode ser tecnicamente eliminada por completo — `docs/content-defense-proportional.md` consolida controles e limites; a marca d’água usa ID interno mínimo; TypeScript e 382 arquivos/966 testes aprovados
- [x] Documentar e verificar a camada de infraestrutura de hospedagem, distinguindo proteção de rede do provedor e controles obrigatórios da própria aplicação — `docs/hosting-security-boundary.md` separa controles verificados no projeto de garantias que dependem de documentação ou painel do provedor, sem exigir configuração manual externa
- [x] Suspender expansões de conteúdo e animação até concluir autorização de servidor, período gratuito de dez lições, limitação de automação e auditoria mínima de acesso — condição concluída com portões curriculares no servidor, avaliação de 10 lições/14 dias, pausa temporária contra automação, inventário de dados e defesa proporcional documentada; expansões futuras continuam condicionadas às regressões existentes
- [x] Auditar e fechar no servidor todo procedimento que entregue texto, exercício, vocabulário ou diálogo de lição sem autorização de conta, aceite de proteção e limite do teste gratuito — regressão audita ABC, Study Base, Pareto, material canônico, geração, exercícios, livro, lição de cena e autorização da lição; todas as rotas exigem `protectedProcedure`. TypeScript e 369 arquivos/935 testes aprovados
...
- [x] Não publicar nova expansão curricular enquanto dados de cartilha, Pareto ou blocos de linguagem permanecerem embutidos em pacotes públicos do navegador — a fronteira curricular exige catálogo de prévias sem hotspots/diálogos, proíbe imports das sementes do servidor e regressões verificam a ausência de material pedagógico no cliente antes de qualquer publicação
- [x] Auditar separadamente evidências de possível exposição de dados pessoais: inventário de dados coletados, armazenamento, acessos administrativos, exportações, registros técnicos e configurações; não inferir vazamento a partir do gráfico agregado — `docs/personal-data-inventory.md` delimita as evidências, controles e lacunas; regressão parental impede IP, agente de usuário e contato identificador em notificação; TypeScript e 380 arquivos/962 testes aprovados
- [x] Revisar a exportação CSV de moderação e bloquear dados pessoais ou conteúdo integral em arquivos administrativos — CSV agora contém somente timestamp, tipo e severidade de violação, faixa etária, score e indicadores de bloqueio/reformulação; regressão dedicada bloqueia IDs, país, religião e conteúdo de conversa
- [x] Reduzir a consulta administrativa de conversas de moderação para impedir a visualização de conteúdo integral, IDs de aluno e categorias sensíveis fora de uma investigação aprovada — consulta e interface expõem apenas tipo de interação, faixa etária, score, status e horário; TypeScript e 378 arquivos/957 testes aprovados
- [x] Reduzir alertas pendentes de moderação para impedir a visualização de conteúdo detectado e identificadores de aluno no painel administrativo — consulta e interface expõem somente classificação, status, ação e horário de revisão; TypeScript, 379 arquivos/959 testes e verificação visual aprovados
- [x] Adicionar regressão de minimização para garantir que o aceite de termos não colete selfie, IP ou agente de usuário na interface ou rota de conformidade — a rota aceita só confirmações necessárias, a etapa foi renomeada para privacidade e a interface declara que selfie, foto de documento, número da máquina e rastreamento não são solicitados. TypeScript e 376 arquivos/953 testes aprovados
...
- [x] Executar auditoria sequencial de segurança — rotas, autenticação, aceite, período de teste, endpoints, automação, registros e infraestrutura; entregar achado, evidência, correção e limite técnico de cada etapa — `docs/security-audit-sequence.md` consolida os controles verificados e seus limites sem declarar segurança absoluta
- [x] Reforçar proteção de contas de avaliação: sessões revogáveis, validade temporal, limites de uso por conta e bloqueio temporário por comportamento anômalo, sem expor e-mail, conteúdo curricular ou dados de menores — acesso de avaliação revogável no aplicativo, expiração de 14 dias, limite de 10 lições e pausa de 15 minutos após 12 novas liberações em 60 segundos; a sessão OAuth externa permanece fora do escopo e está documentada; TypeScript e 392 arquivos/988 testes aprovados
- [x] Permitir a revogação voluntária e persistente do acesso de avaliação, bloqueando novas entregas curriculares sem prometer controle da sessão OAuth externa — novo estado `revoked`, migração aplicada, portão pedagógico diferenciado e limite documentado; TypeScript e 388 arquivos/982 testes aprovados
- [x] Expor no painel o controle voluntário de encerramento da avaliação com confirmação explícita, sem afetar assinatura ativa ou sessão OAuth externa — painel efetivo consulta o limite de 10 lições, exige confirmação antes da mutação de revogação e foi verificado visualmente; TypeScript e 389 arquivos/985 testes aprovados
- [x] Remover do painel a chamada comercial com preço e checkout até haver aprovação explícita do proprietário — os dois painéis não exibem checkout nem preço, comunicam a proteção comercial e foram cobertos por regressão; TypeScript, 390 arquivos/986 testes e verificação visual aprovados
- [x] Substituir no painel o aviso técnico de provedor local indisponível por comunicação positiva sem declarar IA local ativa — o painel mostra assistência de estudo e deixa integração local como opção futura validada; TypeScript, 391 arquivos/987 testes e verificação visual aprovados
- [x] Alinhar o painel aos seis idiomas comerciais iniciais, removendo a alegação desatualizada de 58 idiomas ativos enquanto os demais continuam em preparação — o painel apresenta PT-BR, EN, ES, FR, IT e DE como trilhas iniciais, inglês A1 como piloto e catálogo de 143 idiomas; TypeScript, 392 arquivos/988 testes e verificação visual aprovados
- [x] Restringir o atalho de painel de vendas a administradores para que alunos não recebam acesso visual a controles comerciais — o link só é renderizado para `user.role === admin`; TypeScript e 393 arquivos/989 testes aprovados
- [x] Corrigir o painel que ainda apresenta cinco lições e 69 idiomas, alinhando-o aos 10 acessos de avaliação e 143 idiomas suportados — o painel legado passou a consultar o limite de avaliação e declara 143 idiomas sem prometer currículo integral; o painel efetivo foi verificado visualmente com 10 lições e 143 idiomas; TypeScript e 389 arquivos/984 testes aprovados
- [x] Bloquear temporariamente tentativas anômalas de autorizar novas lições de avaliação, sem persistir conteúdo, IP, e-mail ou dados de menores — após 12 novas liberações em 60 segundos, a conta é pausada por 15 minutos em memória; lições já autorizadas não são afetadas; TypeScript e 381 arquivos/965 testes aprovados
- [x] Substituir a marca d’água nominal de conteúdo protegido por um identificador interno mínimo, sem nome ou e-mail do aluno — lições protegidas exibem somente “Acesso protegido” e o ID interno da conta quando aplicável; TypeScript, 382 arquivos/966 testes e rota de lição verificados
- [x] Auditar expiração e limpeza proporcional de sessões de IA — `ai_sessions` não possui consumidor ativo no servidor ou cliente; não há contexto de conversa sendo persistido por esse esquema. Qualquer ativação futura deve incluir TTL, limpeza proporcional e regressão antes de gravar conteúdo
...
- [x] Aplicar cabeçalhos modernos de proteção no aplicativo: política de conteúdo, bloqueio de enquadramento, restrição de recursos do navegador e política de referência, com testes para não prejudicar autenticação, mídia ou rotas públicas — CSP restritiva com enquadramento oficial, nosniff, referrer policy, permissions policy, HSTS, mídia permitida e política de domínio cruzado são cobertos por regressão. TypeScript e 370 arquivos/936 testes aprovados
...
- [x] Definir e implementar retenção/revisão do documento e contato parental protegidos no banco, com prazo, finalidade e exclusão proporcional após expiração ou inatividade — rotina usa janela fixa de 30 dias e limpa apenas documento/e-mail opcionais após revogação ou inatividade; regressão dedicada aprovada
- [x] Implementar retenção mínima dos contatos parentais opcionais: preservar apenas o consentimento e suas confirmações enquanto ativos, expirar documento/e-mail opcionais após prazo definido e limpar dados de consentimentos revogados sem alterar o bloqueio de acesso — SQL preserva nome, vínculo, versões, confirmações e estado do consentimento
- [x] Aplicar o prazo confirmado de 30 dias: após revogação ou inatividade, apagar apenas `guardian_document` e `guardian_email`; preservar nome, vínculo, data, versão e estado mínimo de consentimento para prestação de contas, sem restaurar acesso — teste confirma cutoff, campos limpos e ausência de alteração no direito curricular
...
- [x] Implementar primeiro o painel interno de resumos de segurança, atividade, sugestões e críticas, acessível somente ao responsável e sem transferência de dados a serviços externos — o painel parental consulta somente perfis vinculados ao responsável autenticado; o roteador aplica `requireChildOwnership` a configurações, alertas, atividade e revogação; a interface expõe horário, idioma, tipo e alerta, sem mensagens, respostas ou transcrições; a suíte de 393 arquivos/989 testes aprovou
...
- [x] Tornar explícito no contrato administrativo que geração e diagnóstico retornam somente propostas revisáveis, sem aplicar código, configuração ou publicação — ambas as mutações administrativas retornam `executionMode: proposal_only` e exigem aprovação humana; regressão bloqueia operações de escrita/publicação; TypeScript e 394 arquivos/991 testes aprovados
- [x] Definir blocos independentes de idioma para inglês, espanhol, francês, italiano e alemão: contrato público de disponibilidade preparado, com inglês como piloto A1 e os demais blocos em preparação; aguarda compilação, regressões e validação visual antes de marcar como concluído. — contratos centralizados usam locale canônico, fronteira de conteúdo específico no servidor e proíbem fallback curricular entre idiomas; TypeScript e 383 arquivos/969 testes aprovados
- [x] Garantir que cada bloco de idioma aceite qualquer idioma nativo disponível como apoio: o contrato declara apoio a qualquer idioma nativo ativo; aguarda compilação, regressões e validação visual antes de marcar como concluído. — os cinco contratos usam `any-available-native-language`; a regressão cobre variantes regionais de locale e o onboarding foi aberto visualmente
- [x] Expandir os cinco blocos iniciais com unidades autorais inspiradas somente nos objetivos pedagógicos da referência, conectando vocabulário Pareto, diálogo, escrita, perguntas, professor e revisão sem reutilizar texto, exercícios, imagens ou sequência distintiva — as 10 unidades A1 específicas foram entregues exclusivamente pelo servidor e integradas visualmente à Base de Estudos; regressões cobrem estrutura, isolamento por idioma e bloqueio anônimo; TypeScript e 385 arquivos/974 testes aprovados
- [x] Adicionar duas unidades A1 autorais específicas para cada bloco inicial, com objetivo Pareto, diálogo, escrita, pergunta e foco de revisão no servidor — 10 unidades protegidas foram entregues por `curriculum.commercialLanguageUnits`, sem fallback entre idiomas; TypeScript e 384 arquivos/972 testes aprovados, incluindo bloqueio anônimo
- [x] Exibir as unidades A1 autorizadas do idioma-alvo ativo na Base de Estudos, com vocabulário, diálogo, prática escrita, pergunta e orientação docente — a Base de Estudos consulta o idioma-alvo ativo no servidor, mostra as duas unidades autorizadas e foi verificada visualmente com inglês; TypeScript e 385 arquivos/974 testes aprovados
...
- [x] Impedir que o status do motor facial declare lip-sync ativo apenas pela presença de credencial, sem teste real de geração e revisão de mídia — o status permanece indisponível, geração e consulta externas são bloqueadas por pré-condição e retratos estáveis continuam como padrão; TypeScript e 386 arquivos/976 testes aprovados
- [x] Bloquear o adaptador D-ID de gerar animação facial externa até validação humana, para que nenhuma rota alternativa inicie uso não aprovado — saúde e configuração reportam indisponibilidade; as três rotas de geração retornam pré-condição antes de qualquer chamada externa; TypeScript e 387 arquivos/978 testes aprovados
- [x] Projetar prova de conceito isolada de sincronização labial por áudio, com requisitos de GPU, privacidade, geração de vídeo, qualidade visual e retorno seguro; não substituir a Cena publicada até validação humana — `docs/facial-sync-validation-gate.md` define ensaio descartável, critérios de aprovação e bloqueio em cada falha; TypeScript e 386 arquivos/976 testes aprovados
- [x] Projetar arquitetura híbrida de professores: vídeos pré-gerados para frases pedagógicas essenciais sem GPU do aluno, áudio com retrato estável para conteúdo dinâmico e futura GPU externa opcional com consentimento, privacidade e custo transparentes — `docs/facial-sync-validation-gate.md` define as três camadas, consentimento explícito e retorno ao retrato em qualquer indisponibilidade
- [x] Definir catálogo de vídeos pré-gerados para todas as cenas imersivas: abertura, objeto-chave, pronúncia, instrução, repetição, correção e encerramento por cena/professor/idioma; documentar custos de produção, armazenamento e entrega sem habilitar cobrança ou GPU — `docs/pre-generated-clips-catalog.md` cobre as 29 cenas, sete slots e as regras de armazenamento/entrega sem ativar mídia, GPU ou cobrança; TypeScript e 386 arquivos/976 testes aprovados
- [x] Planejar clipes curtos montados por poses consistentes do professor: apontar, incentivar, corrigir e encerrar; usar somente retratos autorizados, não simular sincronização labial e não iniciar geração de imagem/vídeo sem confirmação do primeiro plano — o catálogo fixa os gatilhos e fallbacks por pose, exige par exato de fala–áudio e mantém respostas livres no retrato estável
...
- [x] Concentrar configuração de GPU/motor facial no aviso inicial e manter as cenas somente com estados visuais: pose neutra, saudação, apontar, incentivo, correção e encerramento; sem textos técnicos repetidos no fluxo pedagógico — a Cena Imersiva não expõe GPU, CUDA, Ollama, LM Studio ou runtime local; a orientação fica na página separada de IA para estudo e a regressão mantém retrato estável até validação facial real; TypeScript e 395 arquivos/993 testes aprovados
...
- [x] Separar a correção imediata de áudio do diálogo da futura prova de conceito facial; não bloquear o diálogo atual nem declarar movimento labial até ambiente com GPU e validação visual real — a boca sintética permanece desativada, o áudio atual continua independente e clipes aprovados só iniciam no `audio.onplaying`; regressão dedicada, TypeScript e 396 arquivos/995 testes aprovados; Praia Tropical verificada visualmente sem aviso técnico e com James visível
...
- [x] Exibir o controle nativo de áudio assim que uma fala estiver preparada, antes de `play()` ou abertura do diálogo, para manter repetição e volume acessíveis em qualquer falha audível — o player nativo torna-se visível com `dialogAudioSource`, preserva volume e repetição antes do autoplay e é coberto por regressões; TypeScript e 397 arquivos/996 testes aprovados
- [x] Corrigir retorno indevido à primeira tela em falhas recuperáveis, preservando rota, cena e estado de estudo do aluno — removido o redirecionamento global por 401 transitório; a rota de Praia Tropical permaneceu aberta após recarga do cliente, com regressão de navegação e TypeScript/398 arquivos/997 testes aprovados
- [x] Impedir que consultas ou mutações com 401 transitório redirecionem globalmente o navegador e descartem a rota atual — assinantes globais de query e mutation não usam mais `window.location` ou login automático; rotas preservam estado e tratam acesso localmente; TypeScript e 398 arquivos/997 testes aprovados
- [x] Impedir que a ativação de um service worker recarregue automaticamente a página e reinicie uma cena ou lição em andamento — a atualização assume o controle sem `window.location.reload()`, preservando rota e estado até a próxima abertura; TypeScript e 399 arquivos/998 testes aprovados; Praia Tropical confirmada visualmente na rota após o reforço
...
- [x] Estabilizar o botão Ouvir James: preservar fonte Blob persistente, evitar estados contraditórios após autoplay bloqueado e manter uma única orientação até confirmação auditiva humana — o player mantém `dialogAudioSource`, volume e repetição; o aviso usa um único estado renderizado uma vez no diálogo; a confirmação humana de audição permanece bloqueadora no item separado
- [ ] Priorizar correções internas testáveis que não dependam de dispositivo, credencial ou aprovação externa, mantendo validações humanas registradas sem bloquear a sequência
- [ ] Executar as pendências internas em ordem de impacto e independência, sem aguardar tarefas que dependem de mídia nova, credencial externa ou confirmação no dispositivo
- [ ] Remover do diagnóstico agendado a noção de correção automática e exigir que toda recomendação seja somente proposta revisável
- [x] Compor em um único resolvedor interno a identidade docente canônica e a política segura de mídia, sem ativar clipe novo nem alterar o professor exibido — `resolveCanonicalTeacherResource` preserva James/retrato/voz em PT-BR→EN e aplica retrato estável por padrão; TypeScript e 400 arquivos/1000 testes aprovados; Praia Tropical verificada visualmente com James canônico
...
- [ ] Consolidar em uma única guarda o par validado de fala, áudio e clipe de James para impedir que correções posteriores removam novamente o movimento aprovado
- [x] Restaurar a renderização do clipe lateral aprovado quando a política confirmar movimento temporizado pelo mesmo áudio, sem alegar sincronização labial — a guarda aceita `audio_timed_motion_video`, James prepara o clipe apenas na Praia Tropical e ele só aparece depois de `audio.onplaying`; TypeScript e 403 arquivos/1006 testes aprovados
...
- [ ] Corrigir o motor compartilhado que deixa o player em 0:00 e o retrato estático em cenas como Praia Tropical e Parque da Cidade, sem cruzar mídia entre professores
- [ ] Aplicar regressão matricial às 29 cenas para preservar preparação de áudio, duração válida, professor canônico e política de mídia já aprovados
- [ ] Bloquear por contrato único qualquer mudança que reintroduza player 0:00, retrato fora da identidade canônica ou mídia incompatível em uma das 29 cenas
- [ ] Ampliar responsabilidades de revisão externa, diagnóstico local e validação interna para detectar, explicar e bloquear regressões sem conceder alterações autônomas
- [ ] Preservar por contrato as configurações validadas de fontes de IA, áudio, professor e mídia, bloqueando reintrodução de defeitos antes de publicação
- [ ] Ampliar execução assistida por IA em correções, conteúdo, testes e preparação de mídia, exigindo evidência e validação antes de qualquer mudança sensível
...
- [x] Materializar quatro níveis pedagógicos: inicial, intermediário, avançado e tecnológico, com critérios verificáveis de conteúdo, interação e domínio para cada passagem — o painel protegido exibe faixas CEFR, domínio mínimo, evidências explícitas e estado de conteúdo; TypeScript e 416 arquivos/1035 testes aprovados, painel verificado visualmente
- [ ] Integrar a passagem por nível ao progresso protegido do aluno, sem liberar conteúdo tecnológico não aprovado
- [x] Derivar domínio real do SRS protegido na prontidão pedagógica — a rota getCourseProgress consulta totalCorrect e totalWrong do SRS do servidor, calcula averageMastery quando há 5 ou mais respostas e mantém canUnlockCurriculum: false; XP e lições não alteram o nível pedagógico; TypeScript e 416 arquivos/1039 testes aprovados
- [x] Exibir domínio SRS derivado no painel do aluno — quando há 5 ou mais respostas avaliadas, o painel mostra "Domínio derivado do SRS" com percentual e nota que não libera currículo; sem respostas suficientes, exibe "ao menos 5 respostas avaliadas pelo servidor"; TypeScript e 416 arquivos/1039 testes aprovados, painel verificado visualmente
- [x] Entregar na rota protegida de progresso uma prontidão pedagógica derivada de lições e pontos reais, sem interpretar XP como desbloqueio curricular — `progress.getCourseProgress` retorna domínio médio, nível atual, próxima etapa e evidência pendente; nunca libera conteúdo por XP; TypeScript e 411 arquivos/1021 testes aprovados
...
- [x] Codificar o nível tecnológico e os critérios de passagem entre os quatro níveis, mantendo conteúdo tecnológico específico como pendência até haver unidades aprovadas — o contrato protegido exige 90% de domínio e evidências de transferência/aplicação, e o painel mantém a etapa como planejada até unidades aprovadas; TypeScript e 416 arquivos/1035 testes aprovados
- [x] Impedir que pontos motivacionais ou quantidade de lições simulem domínio ou passagem pedagógica — o progresso protegido distingue faixa observada por lições de etapa confirmada, aguarda respostas avaliadas para medir domínio e mantém `canUnlockCurriculum: false`; TypeScript e 416 arquivos/1036 testes aprovados
- [x] Limpar fonte de áudio inválida antes do fallback local para evitar player residual, repetição de erro ou travamento da cena — a recuperação preserva o professor, o retrato e os controles de fala válida; TypeScript e 416 arquivos/1037 testes aprovados, Cena Praia Tropical verificada visualmente
...
- [x] Exibir no painel do aluno a prontidão pedagógica protegida do curso selecionado, distinguindo domínio, evidência pendente e XP sem criar desbloqueio pela interface — consulta protegida por curso, domínio e etapas visíveis, regressão dedicada, TypeScript e 412 arquivos/1023 testes aprovados; painel verificado visualmente
...
- [x] Impedir que a preferência do idioma alvo inverta a dificuldade da galeria de cenas — a galeria preserva beginner→intermediate→advanced e prioriza o idioma somente dentro de cada etapa; professores, retratos e política de áudio não foram alterados; TypeScript e 413 arquivos/1026 testes aprovados, Cena Praia Tropical verificada visualmente
- [x] Usar a jornada pedagógica central também no controle Próxima da Cena Imersiva — o avanço mantém beginner→intermediate→advanced e prioriza idioma alvo somente na mesma etapa, sem alterar professor, retrato ou áudio; TypeScript e 416 arquivos/1037 testes aprovados, Cena Praia Tropical verificada visualmente
- [x] Conectar às cenas autorizadas o roteiro de conceito, prática guiada, resposta, correção e aplicação — a entrega usa rota curricular protegida, mantém a exigência de mídia docente compatível e apresenta o roteiro no diálogo sem alterar professor ou áudio; TypeScript e 414 arquivos/1029 testes aprovados
- [x] Proteger por regressão a sequência visível de conceito, prática guiada, resposta, correção e aplicação na Cena Imersiva — a etapa ativa acompanha o diálogo autorizado, usa foco progressivo por dificuldade e preserva a exigência de professor visível com fala compatível; TypeScript e 416 arquivos/1038 testes aprovados
- [x] Entregar orientação concreta por etapa no roteiro pedagógico protegido — conceito, prática guiada, resposta, correção e aplicação recebem instruções distintas para níveis inicial, intermediário e avançado, e a Cena Imersiva mostra a orientação da etapa ativa sem alterar professor, retrato ou áudio; TypeScript e 416 arquivos/1038 testes aprovados, Cena Praia Tropical verificada visualmente
- [x] Entregar contrastes semânticos do Livro SOS no Pareto contextual das cenas autorizadas — praia: ever/always; aeroporto: there/their/they're; Nova York: say/tell; museu: look up/look for; pares de idioma ou cenas sem conteúdo aprovado retornam vazio, sem conteúdo fabricado; TypeScript e 415 arquivos/1032 testes aprovados
- [x] Expandir progressivamente contrastes do Livro SOS para novas cenas PT-BR→EN autorizadas — family home: bring/take; café: fun/funny; restaurante: few/little; hotel: borrow/lend; escritório: actually/currently; cada contraste mantém contexto, compreensão e bloqueio para pares não aprovados; TypeScript e 416 arquivos/1037 testes aprovados, Café Parisiense verificado visualmente
- [x] Ampliar homófonos e locuções no Livro SOS e no Pareto protegido das cenas PT-BR→EN autorizadas — cozinha: turn on/turn off; escola: wear/where; biblioteca: hear/here; metrô: get on/get off; academia: work out/work on; todos mantêm significado contextual e verificação de compreensão; TypeScript e 416 arquivos/1038 testes aprovados, Cozinha Moderna verificada visualmente
- [x] Ampliar palavras próximas e estruturas de sentido no Livro SOS e no Pareto protegido das cenas PT-BR→EN autorizadas — parque: listen/hear; cinema: see/watch; hospital: sick/ill; supermercado: some/any; Tóquio: travel/trip/journey; todos incluem contraste contextual, forma formal prioritária quando aplicável e verificação de compreensão; TypeScript e 416 arquivos/1038 testes aprovados, Tóquio verificado visualmente
- [x] Completar cobertura contextual nas 29 cenas PT-BR→EN autorizadas — todas as cenas beginner, intermediate e advanced agora entregam um contraste do Livro SOS ao Pareto protegido, com progressão por nível, pergunta de compreensão e bloqueio para pares não aprovados; TypeScript e 416 arquivos/1038 testes aprovados, Jardim Japonês verificado visualmente
...
- [x] Definir no servidor o contrato de contraste semântico por nível, sem inserir pares de vocabulário protegidos no pacote público — a entrega ordena níveis inicial→intermediário→avançado de forma estável no servidor e a regressão impede imports curriculares pelo cliente; TypeScript e 416 arquivos/1038 testes aprovados
...
- [x] Bloquear a promoção do clipe lateral de James quando a fala em reprodução divergir do diálogo canônico do clipe — a guarda exige igualdade textual antes do evento real de áudio, limpa a pendência incompatível e preserva o retrato; TypeScript e 416 arquivos/1038 testes aprovados, Cena Praia Tropical verificada visualmente
...
- [x] Ocultar hotspots e cartões de Pareto quando a Cena Imersiva informar que o acesso protegido ainda está pendente ou falhou — hotspots, cartões, prática Pareto, quiz e diálogo exigem `canUseAuthorizedSceneInteractions`; o tradutor nativo também usa somente `activeSceneDialog`, sem fallback ao catálogo público. TypeScript e 361 arquivos/908 testes aprovados
- [x] Substituir a orientação padrão contraditória da Cena Imersiva para não pedir ativação quando o material canônico já está disponível — a orientação padrão informa que a cena está pronta para explorar os objetos; avisos de ativação ficam restritos ao estado protegido pendente. Regressão de fluxo compartilhado e 361 arquivos/908 testes aprovados
...
- [x] Impedir que a validação e a abertura legítima de cenas imersivas recebam respostas de sobrecarga ou acionem a recuperação local antes de renderizar o cenário — limite global de páginas e ativos permite burst de cenas sem liberar APIs; bloqueio de DDoS permanece acima de 2.000 ativos/minuto. Regressão, TypeScript, 366 arquivos/926 testes e abertura paralela de quatro cenas aprovados
- [x] Corrigir os rótulos e conteúdos pedagógicos remanescentes em árabe e polonês nas cenas Deserto e Fazenda quando o par selecionado for PT-BR→EN — variantes protegidas em inglês preservam as sementes originais de árabe e polonês para seus próprios pares; regressão e verificação visual aprovadas. TypeScript e 362 arquivos/912 testes aprovados
- [x] Corrigir os rótulos e conteúdos pedagógicos remanescentes em polonês na cena Biblioteca quando o par selecionado for PT-BR→EN — variante protegida em inglês preserva a semente polonesa para seu par próprio; regressão e verificação visual aprovadas. TypeScript e 362 arquivos/913 testes aprovados
- [x] Corrigir os rótulos e conteúdos pedagógicos remanescentes em russo e francês nas cenas Escritório e Metrô quando o par selecionado for PT-BR→EN — variantes protegidas em inglês preservam as sementes russa e francesa para seus pares próprios; regressão e verificação visual aprovadas. TypeScript e 362 arquivos/915 testes aprovados
- [x] Corrigir em lote as demais sementes não inglesas da matriz PT-BR→EN, preservando cada versão original para seu par de idioma próprio — Jardim, Café, Paris, Cozinha, Restaurante, Supermercado, Tóquio, Mercado Medieval, Porto, Museu, Parque e Montanha agora recebem diálogo e hotspots protegidos em inglês; regressão cobre todo o catálogo, verificações visuais representativas aprovadas. TypeScript e 362 arquivos/916 testes aprovados
- [x] Remover da página de pré-lançamento a prova social e a contagem de adesões não verificáveis — avatares genéricos e a alegação de 47 adesões foram removidos; regressão e verificação visual aprovadas
- [x] Restaurar uma rota navegável para a página de pré-lançamento, que atualmente retorna 404 — `/pre-launch` e `/prelaunch` apontam para a mesma página; regressão e verificação visual aprovadas
- [x] Impedir que o Caderno inicie áudio automaticamente durante a revelação de tradução ou ao abrir uma anotação — TTS, controles de ouvir e fala na revelação foram removidos; filtros, notas e quiz silencioso foram preservados. TypeScript e 365 arquivos/922 testes aprovados
- [x] Corrigir o rótulo residual “Ouvir resposta de James” para exibir o professor ativo da matriz em todas as cenas — botão agora deriva James ou Ingrid de `teachingScene`; regressões de diálogo e matriz aprovadas. TypeScript e 366 arquivos/926 testes aprovados
- [x] Corrigir os rótulos fixos de reprodução e posição de áudio que ainda mencionam James em cenas de Ingrid — grupo, botão, título e posição de áudio agora derivam o professor ativo; contratos de áudio da Praia e matriz docente preservados. TypeScript e 366 arquivos/926 testes aprovados
- [x] Corrigir os avisos de áudio fixos de James para usar o professor ativo em qualquer cena — mensagens de faixa pronta e reprodução bloqueada agora derivam James ou Ingrid de `teachingScene`; reservas específicas da Praia permanecem separadas. TypeScript e 366 arquivos/926 testes aprovados
- [x] Adicionar regressão permanente que bloqueie conteúdo pedagógico no catálogo público de cenas antes de autorização — catálogo de 29 prévias exige `dialog` e `hotspots` vazios, proíbe imports das sementes do servidor e impede fallback à prévia no fluxo autenticado. TypeScript e 367 arquivos/929 testes aprovados
- [x] Remover o temporizador em processo do middleware de segurança e manter a limpeza de limites de forma determinística por requisição — limpeza oportunista vence buckets a cada cinco minutos durante requisições, sem serviço persistente; limites de API, autenticação e ativos preservados. TypeScript e 370 arquivos/936 testes aprovados
- [x] Impedir movimento reutilizável quando o catálogo exigir mídia específica da cena, preservando o retrato canônico como fallback — o resolvedor só permite clipe aprovado com professor e áudio confirmados; movimento reutilizável entre cenários permanece desativado. TypeScript e 370 arquivos/937 testes aprovados
- [x] Não usar voz do sistema automaticamente quando a reprodução inicial de James for bloqueada; manter a retomada no botão explícito de ouvir — rejeição inicial mantém a faixa preparada, exige gesto no controle Ouvir e não aciona TTS local; falhas reais de faixa continuam com fallback seguro separado. TypeScript e 370 arquivos/937 testes aprovados
...
- [x] Comunicar benefícios de privacidade, proteção de acesso e continuidade ao cliente sem divulgar regras, limites, rotas ou mecanismos operacionais de segurança — página inicial apresenta acesso por conta, proteção por idade, canal privado e verificações preventivas em linguagem positiva, sem expor controles defensivos; TypeScript e 423 arquivos / 1054 testes aprovados
- [x] Criar canal de feedback discreto em todas as telas, com armazenamento privado e notificação para feedback crítico — botão global Opinião abre conversa privada; relatos de segurança recebem prioridade alta e alerta minimizado ao proprietário; TypeScript e 421 arquivos / 1050 testes aprovados
- [x] Criar painel privado do proprietário para feedbacks, incidentes, métricas de uso e recomendações de manutenção ou vendas que nunca executem ações comerciais automaticamente — Centro de Controle agrega somente contagens de mensagens abertas, relatos de segurança e melhorias, além dos incidentes existentes; decisões e respostas continuam exclusivamente manuais
- [x] Implantar verificação periódica e idempotente de qualidade de cenas, áudio e matriz docente, com relatório privado e sem interromper clientes ativos — a rotina diária já ativa valida as 29 cenas, matrizes docente/voz, prévias e sementes protegidas, grava somente evidência agregada, alerta o proprietário em divergência e nunca altera produção; TypeScript e 422 arquivos / 1053 testes aprovados
...
- [x] Expandir no Livro SOS, gramática e Pareto protegido os contrastes de always, ever, never, already, yet e still, com posição na frase, forma afirmativa/interrogativa/negativa, sentido contextual, exemplos práticos e verificação de compreensão — entrega PT-BR→EN inclui oito exemplos, exercício Pareto e explicação visível na cena autorizada; 419 arquivos / 1047 testes aprovados
- [x] Incluir no currículo protegido PT-BR→EN figuras de linguagem com sentido, registro e exercícios — metáfora, personificação, hipérbole, onomatopeia e pleonasmo enfático foram adicionados ao Livro SOS, com reescrita literal e prática Pareto; 419 arquivos / 1047 testes aprovados
- [x] Localizar e revisar por par de idiomas aprovado as figuras de linguagem e os registros correspondentes, sem traduzir literalmente metáforas, onomatopeias ou expressões culturais de um idioma para outro — entregue para os seis idiomas comerciais iniciais (PT→EN/ES/FR/IT/DE e EN→PT), com exemplos próprios, prática Pareto protegida, alternativa formal e bloqueio explícito dos demais pares até revisão autoral; TypeScript e 428 arquivos / 1063 testes aprovados
- [x] Criar dez lições A1 progressivas de demonstração com conteúdo no servidor: lições 1–3 acessíveis sem cadastro e lições 4–10 exibidas somente como percurso bloqueado com convite claro para o plano, sem entregar currículo bloqueado ao navegador — rota protegida entrega apenas as três primeiras; página exibe as dez etapas, cadeados e CTA do curso completo; TypeScript e 424 arquivos / 1056 testes aprovados, fluxo validado visualmente
- [x] Criar uma amostra gratuita e limitada da Cena Praia Tropical, com no máximo três interações públicas e convite para continuar por plano ou conta, preservando James, sua voz en-US masculina e os pares mídia+fala aprovados — rota /demo-scene entrega três turnos próprios de amostra, usa retrato estável de James, voz en-US masculina e CTA após a terceira interação, sem carregar o diálogo canônico protegido; TypeScript e 425 arquivos / 1058 testes aprovados, fluxo validado visualmente
- [x] Ampliar o painel privado do proprietário com série agregada de uso, incidentes e feedbacks, além de recomendações revisáveis de manutenção e vendas que não executem qualquer ação comercial automaticamente — o Centro de Controle mostra sete dias de contagens de recursos assistidos, incidentes e retornos, interesse comercial agregado e sugestões que exigem revisão manual; nenhum conteúdo, identificação ou ação comercial automática é retornado; TypeScript e 426 arquivos / 1059 testes aprovados
- [x] Tornar a demonstração A1 e a amostra limitada da Praia Tropical acessíveis pela página inicial e pela navegação pública, sem alterar os preços, iniciar cadastro automático ou liberar conteúdo bloqueado — atalhos no menu Plataforma e no cartão inicial levam a /demo e /demo-scene; TypeScript e 429 arquivos / 1064 testes aprovados, interface validada visualmente
